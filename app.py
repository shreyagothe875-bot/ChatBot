...
if __name__ == '__main__':
    app.run(debug=True, port=5001)from flask import Flask, render_template, request, jsonify, session, redirect, url_for
from google import genai
from google.genai import types
import sqlite3
import json
...
# --- API Configuration ---
# Fetch from Vercel Environment Variables to keep it safe on GitHub
API_KEY = os.environ.get("GEMINI_API_KEY") 
client = genai.Client(api_key=API_KEY)

# --- Vercel Serverless Database Fix ---
# Vercel's server is read-only. It ONLY allows saving files in the /tmp directory.
DB_PATH = '/tmp/sys_users.db'
...
def generate_flashcards():
    ...
    try:
        ...
        prompt = f"Generate 5 educational flashcards as JSON for: {row[1]}"
        response = client.models.generate_content(
            model="gemini-2.0-flash", # Updated to match your chat route for consistency
            contents=prompt,
            config=types.GenerateContentConfig(response_mime_type="application/json")
        )
        return jsonify(json.loads(response.text))
